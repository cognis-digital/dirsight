# Comprehensive installer for cognis-digital/dirsight (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# dirsight is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "dirsight"
$Url  = "git+https://github.com/cognis-digital/dirsight.git"
$Git  = "https://github.com/cognis-digital/dirsight.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: dirsight"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: dirsight"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: dirsight"; exit 0 }
}
